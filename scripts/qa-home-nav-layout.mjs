...
const closed=await snap();if(!await page.locator('[data-review-rotator]').isVisible())failures.push('review-hidden-'+width);console.log('initial',width,JSON.stringify(closed));await page.screenshot({path:`tmp/agy-${width}.png`,fullPage:false});await page.locator('.ss-menu-button').click();const opened=await snap();await page.locator('#programsDropdownToggle').click();await page.waitForTimeout(200);const dropdown=await page.locator('#programsDropdownMenu').isVisible();await page.locator('.ss-menu-button').click();const reclosed=await snap();
...
if(width===390||width===1366){const author=await page.locator('#ss-review-author').innerText();await page.waitForTimeout(7600);if(author===await page.locator('#ss-review-author').innerText())failures.push('review-rotation');}
