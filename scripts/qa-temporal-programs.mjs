...
import temporalPrograms, { getProgramState, getEnrollmentState } from "../src/_data/temporal-programs.js";
...
assert.equal(temporalPrograms.summerAcademy.temporaryContent.owner, "Summer Academy");
assert.equal(new Date(temporalPrograms.summerAcademy.temporaryContent.endsAt).getTime(), new Date(cohort.endAt).getTime());
...
assert.equal(getProgramState("2026-08-21T04:00:00Z", cohort), "ENDED");
