...
Outputs production-quality static ads, multi-slide carousels, and story/reel title cards.
"""
import argparse, html, json, sys, shutil
...
def render_carousel_slide(out_dir, slide_num, total_slides, headline, body_paragraphs, photo_path, logo_img, badge_text=""):
    w, h = 1080, 1350
    im = Image.new('RGB', (w, h), '#151922')
    draw = ImageDraw.Draw(im)

    create_header(im, draw, logo_img, f"BEGINNER LANE • SLIDE {slide_num} OF {total_slides}")
    ...
    file_name = f"slot01_carousel_slide_{slide_num}.png"
...
def main():
    ...
    parser.add_argument('--day', default='Tuesday', help="Day of the week (default: Tuesday)")
    parser.add_argument('--date', default='2026-09-15', help="Calendar date YYYY-MM-DD")
    args = parser.parse_args()

    out_dir = OUTPUT_ROOT / f"tuesday-{args.date}"
    ...
    s2 = render_carousel_slide(
        out_dir, 2, 3,
        "Day 1 is Coached Learning. Zero Live Sparring.",
        [
    ...
    s3 = render_carousel_slide(
        ...
        [
            ...
            "• Clean mats, zero meathead ego, stroller-friendly studio",
...
<title>SSBJJ Tuesday Render Gallery — {args.date}</title>
<style>
  ...
  .card .desc {{ padding: 14px 16px; font-size: 14px; color: #E0D9D1; }}
  .card .desc strong {{ color: #F3CF52; font-size: 15px; display: inline-block; margin-bottom: 4px; }}
</style>
</head>
<body>
  <h1>🥋 Sensei Sandy BJJ — Tuesday Production Assets ({args.date})</h1>
  <p class="subhead">All 8 visual exports for Tuesday: original conversion reference, 3-slide carousel for Beginner Ben, 2 reel title cards, 1 story graphic, and 1 static visitor feed ad.</p>
  <div class="grid">
    {''.join(html_cards)}
  </div>
</body>
...
"""
    ...
    print(f"Successfully rendered {len(rendered_cards)} graphics for Tuesday ({args.date}): {out_dir.resolve()}")
