...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Modal popup (welcome)
  const popupModal = qs('#popupModal');
  const closeModalBtn = qs('#closeModal');
  if (popupModal && closeModalBtn) {
  ...
  // Exit intent popup
  const exitPopup = qs('#exitPopup');
  const exitFormClose = qsa('.close-popup');
  let exitShown = false;
  const showExitPopup = () => {
  ...
  exitFormClose.forEach((btn) => btn.addEventListener('click', () => {
    if (exitPopup) exitPopup.style.display = 'none';
  }));

  // Exit form submission (no-op unless a real endpoint is configured)
  const exitForm = qs('#exitEmailForm');
  const exitFeedback = qs('#exitFeedback');
  if (exitForm && exitFeedback) {
    exitForm.addEventListener('submit', (event) => {
      event.preventDefault();
      exitFeedback.textContent = 'Email capture not configured yet.';
      exitFeedback.style.color = 'orange';
    });
  }
