...
const names = ['Dates', 'Property', 'Guests', 'Cuisine', 'Drink', 'Bottle Match', 'Icebox', 'House Drop', 'Review'];
let current = 0;
...
function updateSummary() {
  const drink = form.elements.drink?.value || 'Not chosen';
  const size = form.elements.bottleSize?.value || '750ml';
  const guests = form.elements.guests?.value || '6';
  document.querySelector('#hw-summary-list').innerHTML = `<div><dt>Guests</dt><dd>${guests}</dd></div><div><dt>Drink</dt><dd>${drink}</dd></div><div><dt>Bottle Match</dt><dd>${size}</dd></div><div><dt>Spirit</dt><dd>Guest supplied</dd></div>`;
}
function valid() {
...
function render() {
  ...
  updateSummary();
  if (current > 0) send(['hyphen_dates_selected','hyphen_property_entered','hyphen_guest_count_changed','hyphen_cuisine_selected','hyphen_drink_selected','hyphen_size_selected','hyphen_ice_added','hyphen_delivery_selected'][current - 1]);
}
