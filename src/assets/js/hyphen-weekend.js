...
const hero = document.querySelector('#hw-hero-title');
if (hero) hero.innerHTML = 'Cocktail kits, ice, and garnishes for your Catskills weekend.';
const heroLede = document.querySelector('.hw-hero-lede');
if (heroLede) heroLede.textContent = 'Pick your drinks. We prep them. Add your bottle. We can deliver it to the house.';
document.querySelectorAll('.hw-problem,.hw-food,section.hw-house[aria-labelledby="house-title"],.hw-house-context,.hw-apres,.hw-choose-path').forEach(section => section.remove());
const weddingSection = document.querySelector('section.hw-house[aria-labelledby="wedding-title"]');
if (weddingSection) weddingSection.querySelector('div').innerHTML = '<p class="hw-eyebrow">HERE FOR A WEDDING?</p><h2 id="wedding-title">We can stock the house before you arrive.</h2><a class="hw-button hw-button-primary" href="#configurator" data-hw-wedding>BUILD THE HOUSE</a>';
document.querySelectorAll('.hw-pricing[aria-labelledby="flavors-title"] .hw-price-card').forEach(card => { const name = card.querySelector('.hw-price-kicker')?.textContent.trim(); const simple = { Bright: ['Lime', 'Rum · Gin · Tequila'], Silk: ['Sour', 'Gin'], Deep: ['Spirit-Forward', 'Rye · Bourbon'] }[name]; const copy = card.querySelector('p:not(.hw-price-kicker)'); if (simple && copy) copy.innerHTML = '<b>' + simple[0] + '</b><br>' + simple[1]; });
document.querySelectorAll('input[name="finishKit"]').forEach(input => { const label = input.closest('label'); if (label) label.querySelector('b').textContent = 'ADD GARNISH KIT'; });
document.querySelectorAll('input[name="houseDrop"]').forEach(input => { const label = input.closest('label'); if (label) { const name = label.querySelector('b'); if (name) name.textContent = input.value === 'friday' ? 'HOUSE DELIVERY · FRIDAY' : 'HOUSE DELIVERY · SATURDAY MORNING'; } });
const sourcingNames = { 'guest-supplied': 'USE MY BOTTLE', courtesy: 'BOTTLE RUN', curated: 'BOTTLE PICK', priority: 'RUSH / SPECIAL REQUEST' };
document.querySelectorAll('input[name="sourcing"]').forEach(input => { const name = input.closest('label')?.querySelector('b'); if (name) name.textContent = sourcingNames[input.value] || name.textContent; });
const finishLegend = document.querySelector('input[name="finishKit"]')?.closest('fieldset')?.querySelector('legend'); if (finishLegend) finishLegend.textContent = 'Add a Garnish Kit';
const dropLegend = document.querySelector('input[name="houseDrop"]')?.closest('fieldset')?.querySelector('legend'); if (dropLegend) dropLegend.textContent = 'Add House Delivery';
